...
window.onload = function (event) {
    ...
    getEmployeeDetails(CONNECTION_DATA.TENET_UPLOAD_APARTMENT_PHOTOS_PAGE + queryString, function (employee) {
    ...
    for (g = 0; g < uploadButtons.length; ++g) {
        uploadButtons[g].addEventListener('click', function (event) {
            ...
            const endpoint = CONNECTION_DATA.TENET_UPLOAD_APARTMENT_PHOTOS_ENDPOINT + apartmentId;
            sendToServer({
                    method: "POST",
                    headers: {
                ...
                CONNECTION_DATA.TENET_UPLOAD_APARTMENT_PHOTOS_ENDPOINT + apartmentId,
                formData,
                function (response1) {
